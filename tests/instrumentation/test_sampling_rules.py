...
def setup_teardown_global_config():
    ...
    original_context_sampling_rules = list(
...
class TestGetEffectiveSampleRate:
    def test_error_traces_always_get_error_sample_rate(self):
        ...
        rate = _get_effective_sample_rate(
            is_error=True, trace_name="test", trace_module="test_mod"
        )
        assert rate == 0.5
    ...
    def test_context_rule_always_strategy(self):
        ...
        rate = _get_effective_sample_rate(
            is_error=False, trace_name="my_always_trace", trace_module="mod"
        )
        assert rate == 1.0
    ...
    def test_context_rule_never_strategy(self):
        ...
        rate = _get_effective_sample_rate(
            is_error=False, trace_name="my_never_trace", trace_module="mod"
        )
        assert rate == 0.0
    ...
    def test_context_rule_on_error_strategy_with_error(self):
        ...
        rate = _get_effective_sample_rate(
            is_error=True, trace_name="my_on_error_trace", trace_module="mod"
        )
        assert rate == 1.0
    ...
    def test_context_rule_on_error_strategy_no_error(self):
        ...
        rate = _get_effective_sample_rate(
            is_error=False, trace_name="my_on_error_trace", trace_module="mod"
        )
        assert rate == 0.0
    ...
    def test_context_rule_random_strategy_with_rule_sample_rate(self):
        ...
        rate = _get_effective_sample_rate(
            is_error=False, trace_name="my_random_trace", trace_module="mod"
        )
        assert rate == 0.75
    ...
    def test_no_matching_context_rules_fallback_to_global_sample_rate(self):
        GLOBAL_CONFIG.context_sampling_rules = [
            {"pattern": "unmatched", "strategy": SamplingStrategy.ALWAYS.value}
        ]
        GLOBAL_CONFIG.sample_rate = 0.3
        GLOBAL_CONFIG.error_sample_rate = 0.9  # Should be ignored for non-error

        rate = _get_effective_sample_rate(
            is_error=False, trace_name="some_trace", trace_module="some_mod"
        )
        assert rate == 0.3
    ...
    def test_multiple_context_rules_precedence(self):
        ...
        rate = _get_effective_sample_rate(
            is_error=False, trace_name="specific_general_trace", trace_module="mod"
        )
    ...
    def test_pattern_matching_trace_module(self):
        ...
        rate = _get_effective_sample_rate(
            is_error=False, trace_name="some_trace", trace_module="my_module_match"
        )
        assert rate == 1.0

    def test_pattern_no_match(self):
        GLOBAL_CONFIG.context_sampling_rules = [
            {"pattern": "nomatch", "strategy": SamplingStrategy.ALWAYS.value}
        ]
