...
_current_trace_context: ContextVar[Optional[AsyncTraceContext]] = ContextVar("pypss_async_context", default=None)
...
def _setup_sys_monitoring():
    ...
    try:
        ...
        try:
            ...
            for tool_id_candidate in range(2, 20):
                try:
                    monitoring.use_tool_id(tool_id_candidate, "pypss_monitor")
                    _SYS_MONITORING_TOOL_ID = tool_id_candidate
                    break
                except ValueError:
                    logger.debug(f"PyPSS: Tool ID {tool_id_candidate} already in use. Trying next.")
        ...
        logger.info("PyPSS: sys.monitoring enabled for yield tracking and task churn analysis.")
...
def _teardown_sys_monitoring():
    global _MONITORING_ACTIVE, _SYS_MONITORING_TOOL_ID, _registered_yield_callback, _registered_start_callback
    ...
    try:
        ...
        if _SYS_MONITORING_TOOL_ID is not None:
            ...
            monitoring.use_tool_id(_SYS_MONITORING_TOOL_ID, None)
...
class EventLoopHealthMonitor:
    ...
    def start(self):
        ...
        try:
            ...
            logger.info(f"PyPSS: Event Loop Health Monitor started (interval={self.interval}s)")
        except RuntimeError:
            self._running = False
            return
