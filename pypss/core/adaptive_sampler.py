...
class AdaptiveSampler:
    ...
    def _adjust_sample_rate(self):
        ...
        """
        current_time = time.time()
        if current_time - self._last_adjustment_time < GLOBAL_CONFIG.adaptive_sampler_min_interval:
            return
        ...
        if mode == "high_load":
            if self._last_metrics["qps"] > GLOBAL_CONFIG.adaptive_sampler_high_qps_threshold:
                new_rate = GLOBAL_CONFIG.adaptive_sampler_min_rate
            else:
                new_rate = self._calculate_balanced_rate(new_rate)

        elif mode == "error_triggered":
            if self._last_metrics["error_rate"] > GLOBAL_CONFIG.adaptive_sampler_error_threshold:
            ...
            is_stable = (
                self._last_metrics["error_rate"] < GLOBAL_CONFIG.adaptive_sampler_error_threshold / 2
                and self._last_metrics["lag"] < GLOBAL_CONFIG.adaptive_sampler_lag_threshold / 2
                and self._last_metrics["churn_rate"] < GLOBAL_CONFIG.adaptive_sampler_churn_threshold / 2
            )
            if is_stable:
                new_rate = max(
        ...
        if new_rate != self._current_sample_rate:
            logger.info(
                ...
                f"(QPS={self._last_metrics['qps']:.1f}, Err={self._last_metrics['error_rate']:.2f}, "
                f"Lag={self._last_metrics['lag']:.3f})"
            )
    ...
    def _calculate_balanced_rate(self, current_rate: float) -> float:
        ...
        if self._last_metrics["lag"] > GLOBAL_CONFIG.adaptive_sampler_lag_threshold:
            increase_score += 1
        if self._last_metrics["churn_rate"] > GLOBAL_CONFIG.adaptive_sampler_churn_threshold:
            increase_score += 1
        if self._last_metrics["error_rate"] > GLOBAL_CONFIG.adaptive_sampler_error_threshold:
            increase_score += 1
        ...
        if self._last_metrics["lag"] < GLOBAL_CONFIG.adaptive_sampler_lag_threshold / 2:
            decrease_score += 1
        if self._last_metrics["churn_rate"] < GLOBAL_CONFIG.adaptive_sampler_churn_threshold / 2:
            decrease_score += 1
        if self._last_metrics["error_rate"] < GLOBAL_CONFIG.adaptive_sampler_error_threshold / 2:
            decrease_score += 1

        if increase_score > 0:
            return min(
                GLOBAL_CONFIG.adaptive_sampler_max_rate,
                current_rate + GLOBAL_CONFIG.adaptive_sampler_increase_step * increase_score,
            )
